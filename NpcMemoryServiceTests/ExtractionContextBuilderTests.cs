...
// ExtractionContextBuilder projects the live encounter + the NPC profile into the SETTING/PLAYER/YOU digest the
// PROSE+EXTRACT extractor reads before it writes a memory. It replaces a hardcoded facts string, and the whole
// point is grounding and calibration: a memory must be anchored to the real place and witnesses, the NPC's own
// nature must colour it, and the player must be named consistently. The spike's first extractor, given no player
// reference, once recorded the player as "the coward" and baked that one-time insult into a permanent memory.
// These tests pin the properties that guard against that class of rot: the three labelled lines are always
// present (so the extractor always gets its digest), a partial context degrades to real clauses and never the
...
namespace NpcMemoryServiceTests
{
   ...
   public class ExtractionContextBuilderTests
   {
      ...
      // The full projection must carry every anchor the extractor needs at once: the real place name, a witness by
      // name, the player named by the given label, the NPC's own label, and the signed regard that calibrates tone.
      // If any of these drops out, the extractor invents it, which is exactly what the spike set out to stop.
      [Test]
      public void GIVEN_a_full_context_WHEN_projected_THEN_it_carries_place_witness_player_label_regard_and_npc_label()
      {
         ...
         string digest = ExtractionContextBuilder.Project(context, Profile(regard: 12), "Aldric");
      ...
      // witnesses, and no diplomatic standing, most fields are null. The projection must still hand the extractor
      // its three labelled lines (so it always has a digest to read) and must never leak the literal "null" into a
      // clause, or that placeholder becomes the "place" the extractor grounds a permanent memory in.
      [Test]
      public void GIVEN_a_near_empty_context_WHEN_projected_THEN_it_returns_the_three_labels_without_throwing_or_printing_null()
      {
         var context = new EncounterContext();

         string digest = ExtractionContextBuilder.Project(context, Profile());
      ...
      // extraction turn. A degraded digest keeps the second-model chain alive; an exception kills it.
      [Test]
      public void GIVEN_a_null_context_WHEN_projected_THEN_it_still_returns_the_three_labels_without_throwing()
      {
         string digest = ExtractionContextBuilder.Project(null, Profile());
      ...
      // The exact bug that motivated the builder: the extractor, told nothing about how to name the player, wrote
      // "the coward" into the memory. The digest must state the real player label and must never itself introduce an
      // epithet; the extractor can only reuse what the digest gives it, so a clean label here is what keeps the
      ...
      public void GIVEN_a_player_label_WHEN_projected_THEN_the_player_is_named_by_that_label_and_no_epithet_appears()
      {
         ...
         string digest = ExtractionContextBuilder.Project(context, Profile(), "Aldric");
      ...
      {
         string digest = ExtractionContextBuilder.Project(new EncounterContext(), Profile());
      ...
      // The nature descriptor is what CALIBRATES the extractor's tone: a pitiless lord's warmth reads differently
      ...
      {
         string digest = ExtractionContextBuilder.Project(new EncounterContext(), Profile(trait: "Pitiless"), "Aldric");

         digest.Should().Contain("Pitiless");
      }

      // Negative regard must render with its minus sign, not just a bare number the extractor could read as positive.
      // The sign is the single fact that tells the extractor whether the NPC's baseline toward the player is warm or
      ...
      {
         string digest = ExtractionContextBuilder.Project(new EncounterContext(), Profile(regard: -8), "Aldric");
