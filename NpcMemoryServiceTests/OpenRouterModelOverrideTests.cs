...
// Pins how LlmRequest.ModelOverride reaches the OpenRouter wire body. This is what lets the PROSE + INTERPRETER
// mode point ONE call (the action interpreter) at a cheaper model without touching any global model selection, so
// there is no async race with the shared model-mode setting. If the override stops being honoured, every
// interpreter call would silently bill the scene's own (often expensive) model instead of the configured one;
// if a blank override stopped degrading to the resolved model, an unconfigured interpreter would send an empty
...
namespace NpcMemoryServiceTests
{
   ...
   public class OpenRouterModelOverrideTests
   {
      ...
      // The whole point of the property: a set override is the model this one call targets, so the interpreter
      ...
      {
         object body = BuildClient("scene/expensive-model").ToWireFormat(BuildRequest("interpreter/cheap-model"));

         ModelOf(body).Should().Be("interpreter/cheap-model");
      }

      // Blank ActionInterpreterModel in MCM means "reuse the scene's model": a blank override must NOT blank out the
      ...
      // The default (no override supplied) is the load-bearing guarantee that every existing, non-interpreter
