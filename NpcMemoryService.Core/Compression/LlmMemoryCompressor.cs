    ...
    public sealed class LlmMemoryCompressor : IMemoryCompressor
    {
        ...
        public async Task<CompressionResult> CompressAsync(NpcProfile profile, CancellationToken ct = default)
        {
            ...
            LlmRequest request = new LlmRequest {
                ...
                Messages = new[] {new LlmMessage(MessageRole.User, userPrompt)},
            ...
            (IReadOnlyList<int> keepIndices, var summary) = ParseResponse(llmResponse.Content);
            ...
            List<NotableEvent> kept = finalIndices
        ...
        {
            var keepBody = ExtractSection(raw, "KEEP");
            var summary = ExtractSection(raw, "DROP_SUMMARY");

            var indices = new List<int>();
            if (!string.IsNullOrWhiteSpace(keepBody))
                foreach (var token in keepBody!.Split(new[] {',', ' ', '\n', '\r', '\t'},
        ...
        private string BuildSystemPrompt()
        {
            ...
            sb.AppendLine("- ALWAYS keep every Betrayal, Intimacy, Confrontation, Agreement, and Farewell event.");
            sb.AppendLine();
        ...
        private IEnumerable<int> ComputeProtectedIndices(IReadOnlyList<NotableEvent> events)
        {
            ...
            // Most recent N
            for (var i = Math.Max(0, events.Count - RecentEventsAlwaysKept); i < events.Count; i++)
                protected_.Add(i);
            ...
            for (var i = 0; i < events.Count; i++)
            {
                ...
                if (t == NotableEventType.Betrayal || t == NotableEventType.Intimacy || t == NotableEventType.Confrontation || t == NotableEventType.Agreement || t == NotableEventType.Farewell) protected_.Add(i);
            }
