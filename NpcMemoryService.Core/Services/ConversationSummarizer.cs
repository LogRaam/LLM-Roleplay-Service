...
// Tester feedback: memories only registered when the NPC formally closed the
// conversation, because the LLM mostly emits [EVENT] on a natural farewell.
// This summarizer is the fallback — same two-stage pattern as CaptiveSceneSummarizer.
...
namespace NpcMemoryService.Core.Services
{
    ...
    public sealed class ConversationSummarizer
    {
        ...
        {
            string who = string.IsNullOrWhiteSpace(playerName) ? "a visitor" : playerName;
