...
namespace NpcMemoryService.Core.Prompts
{
   ...
   public static class ActionInterpreterPromptBuilder
   {
      ...
      private static string BuildStablePrefix()
      {
         ...
         sb.AppendLine("type: end_conversation");
         sb.AppendLine("[/ACTION]");
         sb.AppendLine("Emit end_conversation ONLY when the reply UNMISTAKABLY ends the meeting: the NPC physically leaves");
         sb.AppendLine("or turns away to their own business and the player is left standing there, the NPC dismisses or sends");
         sb.AppendLine("the player away, storms off, or the PLAYER's own words clearly take their leave. The NPC must actually");
         sb.AppendLine("depart, not merely speak of parting.");
         sb.AppendLine("Do NOT emit it for a line that only SOUNDS like a wind-down but leaves the player room to answer: a");
         sb.AppendLine("reflective or pleasant remark (\"this seems a fine way to end the evening\", \"it grows late\", \"we");
         sb.AppendLine("should speak again\"), a lull, or ANY line where the NPC asks a question, makes a request, or invites a");
