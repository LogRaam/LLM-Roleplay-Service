...
namespace NpcMemoryService.Core.Prompts
{
   ...
   public static class LetterPromptBuilder
   {
      ...
      private static string DescribeReason(LetterReason reason) => reason switch
      {
         ...
         LetterReason.ServiceOffer          => "You are NOT the player's liege, and you are writing to win them into YOUR service. The Context below says whether they serve another realm now or none at all; write to what it says. It also names the enticement you dangle (a purse of gold, a castle of their own, or a company of soldiers) and why you want them at your side. Make your case in your own voice, name that enticement plainly, and be persuasive without grovelling. Do not pretend the matter is settled; this is an offer they may take or leave.",
         ...
         LetterReason.TournamentResult      => "A tournament has just concluded in which both you and the player competed. The Context below tells you exactly how it went BETWEEN you (who bested whom, and in which round) and the spirit in which you write now: to crow over besting them, to congratulate them graciously, or to acknowledge a hard-fought bout. Write in that spirit, naming the bout as the Context gives it, in your own character. Draw ONLY on what the Context states; invent no other detail of the fighting.",
