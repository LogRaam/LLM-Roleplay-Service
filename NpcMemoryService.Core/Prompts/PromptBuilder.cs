   ...
   public sealed class PromptBuilder : IPromptBuilder
   {
      ...
      private static void AppendWitnesses(StringBuilder sb, EncounterContext? context, LeanPromptLevel lean)
      {
         ...
         if (LeanPromptPolicy.Include(PromptSection.WitnessMachineryTeaching, lean))
         {
            ...
            sb.AppendLine();
            sb.AppendLine("WHEN A WITNESS IS PART OF WHAT IS BEING SAID, THEY ANSWER IN WORDS. This is not only");
            sb.AppendLine("when they are directly addressed: if you (the main speaker) or the player name a");
            sb.AppendLine("witness, discuss them, question them, or make them the subject of the current");
            sb.AppendLine("exchange, that witness MUST reply this turn with a SPOKEN line in quotes, in their");
            sb.AppendLine("own voice, not a bare gesture. A silent gesture or a wordless look from a witness who");
            sb.AppendLine("is named, discussed, or clearly part of the moment reads as a snub or the cold");
            sb.AppendLine("shoulder and derails the scene. A gesture may accompany the words, never replace them.");
            sb.AppendLine("Reserve a reaction that is ONLY a gesture for a witness genuinely on the PERIPHERY of");
            sb.AppendLine("this moment (not named, not discussed, no part of what is being said); the instant a");
            sb.AppendLine("witness is drawn into the exchange in any way, give them WORDS.");
            sb.AppendLine();
            ...
            sb.AppendLine();
            sb.AppendLine("DO NOT OVER-READ A BYSTANDER'S SILENCE. A witness genuinely on the periphery (not");
            sb.AppendLine("named, not discussed, not asked anything) may stay silent or offer only a gesture");
            sb.AppendLine("with no offense meant at all: as the main speaker, never treat that silence, quiet,");
            sb.AppendLine("or wordless gesture as a snub, a cold shoulder, an insult, or disrespect. Only a");
            sb.AppendLine("witness who was DIRECTLY asked a question and then pointedly refuses to answer is a");
            sb.AppendLine("snub. Never escalate, cool toward, or end the conversation over a bystander's body");
            sb.AppendLine("language alone.");
